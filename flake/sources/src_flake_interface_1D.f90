...
SUBROUTINE flake_interface ( dMsnowdt_in, I_atm_in, Q_atm_lw_in, height_u_in, height_tq_in,     &
                             U_a_in, T_a_in, e_a_in, q_a_in, P_a_in,                            &
...
REAL (KIND = ireals), INTENT(IN) ::   &
  ...
  I_atm_in                          , & ! Short-wave radiation flux at the surface [W m^{-2}]
  Q_atm_lw_in                       , & ! Long-wave radiation flux from the atmosphere [W m^{-2}]
  height_u_in                       , & ! Height (above the lake surface) where wind is measured [m]
  ...
  e_a_in                            , & ! Water vapour pressure at z=height_tq_in [N m^{-2}]
...
REAL (KIND = ireals), external ::    Qinflow
...
!write(*,*) Q_w_flk
! add the inflow heat flux
Q_w_flk = Q_w_flk + Qinflow(T_sfc_p)
!write(*,*) Q_w_flk
!pause
!
